...
export default function ComponentCard({ componentData }: { componentData: GalsenUiComponentGroup }) {
  return (
    <Link href={`/components/${componentData.slug}`}>
      <div className="group bg-blue-100 dark:bg-gray-800 p-5 font-medium md:flex md:justify-between md:items-center hover:bg-blue-200 dark:hover:bg-gray-700">
        <div className="flex items-center gap-3">
          <span aria-hidden="true" role="img" className="text-lg text-center sm:text-xl">
            {componentData.emoji}
          </span>
          <h4 className="text-neutral-900 dark:text-gray-100" title={componentData.title}>
            {componentData.title}
          </h4>
          <p className="hidden md:block text-blue-600">-</p>
          <p className="ml-auto text-blue-700 dark:text-blue-400 text-sm">{componentData.count} {componentData.count > 1 ? 'Composants' : 'Composant'}</p>
        </div>
